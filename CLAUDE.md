...
- `ontology/rogue.ttl` — ontologia OWL del contenuto: classi (`sr:Room`, `sr:Monster`/`sr:Boss`, `sr:Item` con sottoclassi `Weapon`/`Consumable`/`KeyItem`, `sr:Faction`, `sr:Quest`, `sr:NPC`, `sr:Portal`, `sr:Biome`), object property e datatype property.
- `ontology/runtime.ttl` — ontologia OWL dello stato di partita: `sr:Player`, `sr:currentRoom`, `sr:hasItem`, `sr:isAlive`, `sr:isOpen`, `sr:questStatus`. Convenzione: assenza di tripla = stato di default.
- `dataset/dataset.ttl` — mondo authored a mano (istanze nel namespace `ex:`).
- `shacl/rogue-rules.ttl` — shape SHACL del contenuto, sia property shape classiche sia SPARQL-based (es. stanze con `dangerLevel >= 6` devono ospitare un mostro; chiavi orfane, stanze irraggiungibili, cicli lock-key).
- `shacl/runtime-rules.ttl` — shape SHACL dello stato di partita (posizione univoca del player, valori ammessi di `questStatus`, portali aperti solo con chiave posseduta, provenienza lecita degli oggetti in inventario).
- `queries/*.rq` — query SPARQL di lettura, autonome (dichiarano i propri PREFIX).
- `queries/runtime/*.rq` — query usate dal motore (`tools/engine.py`) sul grafo di una partita in corso; non raccolte da `tools.query --all`.
- `tools/` — CLI Python: `common.py` (caricamento grafi, `load_runtime_world`), `validate.py`, `query.py`, `config.py` (lettura `config.toml`), `engine.py` (motore: stato iniziale, transizioni, validazione runtime), `play.py` (CLI del simulatore).
- `tests/` — pytest; `tests/invalid/` contiene un micro-mondo rotto per ogni shape di contenuto.
